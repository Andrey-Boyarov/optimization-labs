...
public class Matrix
{
    ...
    private ArrayList<DimPoint> rows;

    public DimPoint row(int rowId)
    {
    ...
    public Matrix addCol(DimPoint col)
    {
        if (col.size() != rows())
        {
    ...
    public Matrix addRow(DimPoint row)
    {
        if (row.size() != cols())
        {
    ...
    public ArrayList<DimPoint> getRows() {
        return rows;
    }
    ...
    public Matrix(Matrix original)
    {
        ...
        {
            rows.add(new DimPoint(original.rows.get(i)));
        }
    }

    public static Matrix hessian(Function<DimPoint, Double> f, DimPoint x, double eps)
    {
        ...
        for (row = 0; row < res.rows(); row++)
        {
            ...
            {
                val = DimPoint.partial2(f, x, row, col, eps);
                res.set(row,col,val);
                res.set(col,row,val);
            }
        }
    ...
    public static Matrix hessian(Function<DimPoint, Double> f, DimPoint x)
    {
     ...
     */
    public static SolutionType checkSystem(Matrix A, DimPoint b)
    {
     ...
     * @return
     */
    private static DimPoint linsolve( Matrix low,  Matrix up,  DimPoint b)
    {
        double det = 1.0;
     ...
     * @return
     */
    public static DimPoint linsolve(Matrix mat, DimPoint b)
    {
    ...
    public Matrix sub(Matrix other)
    {
        ...
        for (int i = 0; i < rows(); i++)
        {
            rows.set(i, rows.get(i).minus(other.rows.get(i)));
        }
        return  this;
    }
    ...
    public Matrix sub(double other)
    {
        for (int i = 0; i < rows(); i++)
        {
            rows.set(i, rows.get(i).minus(other));
        }
        return  this;
    }
    ...
    public Matrix mul(double other)
    {
        for (int i = 0; i < rows(); i++)
        {
            rows.set(i, rows.get(i).times(other));
        }
        return  this;
    }
    ...
    public static Matrix mul(Matrix a, Matrix b)
    {
        ...
        for (int i = 0; i < a.rows(); i++)
        {
            ...
            {
                res.set(i,j,DimPoint.dot(a.rows.get(i), b_t.rows.get(j)));
            }
        }
        return res;
    }

    public static DimPoint mul(Matrix mat, DimPoint vec)
    {
        if (mat.cols() != vec.size())
        {
            throw new RuntimeException("unable to matrix and DimPoint myltiply");
        }
        DimPoint result = new DimPoint(mat.rows());
        int cntr = 0;
        for (DimPoint row : mat.rows)
        {
            result.set(cntr++, DimPoint.dot(row, vec));
        }
        return result;
    }

    public static DimPoint mul(DimPoint vec, Matrix mat)
    {
        if (mat.rows() != vec.size())
        {
            throw new RuntimeException("unable to matrix and DimPoint myltiply");
        }
        DimPoint result = new DimPoint(mat.cols());
    ...
    public static Matrix sub(double b, Matrix a)
    {
        ...
        for (int i = 0; i < a.rows(); i++)
        {
            result.rows.set(i, DimPoint.minus(b, a.rows.get(i)));
        }
        return result;
    }
}
