...
public class Simplex
{
    ...
    // вектор ограничений
    private DimPoint boundsVector;

    // вектор стоимостей
    private DimPoint pricesVector;
    ...
    public DimPoint boundsCoefficients()
    {
        return boundsVector;
    }

    public DimPoint pricesCoefficients()
    {
        return pricesVector;
    }
    ...
    public DimPoint currentSimplexSolution(boolean onlyNaturalArgs)
    {
        DimPoint solution = new DimPoint(onlyNaturalArgs ? naturalArgsN() : simplexTable.cols() - 1);
    ...
    public DimPoint currentSimplexSolution()
    {
    ...
    public boolean isPlanOptimal()
    {
        ...
        DimPoint row = simplexTable.row(simplexTable.rows() - 1);
        ...
        if (isTargetFuncModified())
        {
            ...
            DimPoint row_ = simplexTable.row(simplexTable.rows() - 2);
    ...
    private int getMainCol()
    {
        ...
        {
            DimPoint rowAdd = simplexTable.row(simplexTable.rows() - 2);
    ...
    private int[] buildVirtualBasisCol(int inequalityId, Sign inequalitySign)
    {
        if (inequalitySign == Sign.Equal)
        {
            for (int row = 0; row < simplexTable.rows(); row++)
            {
                if (row == inequalityId)
                {
                    simplexTable.row(row).addDim(1.0);
                    continue;
                }
                simplexTable.row(row).addDim(0.0);
            }
    ...
    private void buildSimplexTable()
    {
        ...
        for (int row = 0; row < simplexTable.rows(); row++)
        {
            simplexTable.row(row).addDim(boundsVector.get(row));
        }

        // Построение симплекс разностей

        DimPoint sDeltas = new DimPoint(simplexTable.cols());
        ...
        DimPoint sDeltasAdd = new DimPoint(simplexTable.cols());
        ...
        for (int i = 0; i < fModArgs.size(); i++)
        {
            for (int row = 0; row < simplexTable.rows(); row++)
            {
                ...
                if (simplexTable.get(row,fModArgs.get(i))!= 0)
                {
                    ...
                    simplexTable.row(lastRowId).sub(DimPoint.mul(arg,simplexTable.row(row)));
    ...
    public  String simplexToString()
    {
        ...
        for(DimPoint row : simplexTable.getRows())
        {
            nRow++;
            ...
            for (int col = 0; col < row.size(); col++)
            {
                if (row.get(col) >= 0)
                {
                    sb.append(String.format("|%-12s", " " + DimPoint.toRationalStr(row.get(col))));
                    continue;
                }
                sb.append(String.format("|%-12s", DimPoint.toRationalStr(row.get(col))));
    ...
    public DimPoint solve(SimplexProblemType mode)
    {
        this.mode = mode;

        System.out.println("Simplex problem type : " + mode.toString() + "\n");

        DimPoint solution = new DimPoint(naturalArgsN());
        ...
        while (!isPlanOptimal())
        {
            ...
            for (int i = 0; i < simplexTable.rows(); i++)
            {
                ...
                simplexTable.row(i).sub(DimPoint.mul(simplexTable.get(i,mainCol), simplexTable.row(mainRow)));
            }
            solution = currentSimplexSolution();
            if(showSimplexDebugLog)
            {
                System.out.println("a_main{"+ String.valueOf((mainRow + 1))+", "+ String.valueOf((mainCol + 1)) + "} = " + DimPoint.toRationalStr(aik) + "\n");
                System.out.println(simplexToString());
                System.out.println("current solution: " + DimPoint.toRationalStr(currentSimplexSolution()));
            }
        }
        if (validateSolution())
        {
            ...
            // формирование ответа
            System.out.println("solution: " + DimPoint.toRationalStr(solution));
            return solution;
        }
    ...
    public DimPoint solve()
    {
        return solve(SimplexProblemType.Max);
    }

    public Simplex(Matrix a, DimPoint c, ArrayList<Sign> inequalities, DimPoint b)
    {
    ...
    public Simplex(Matrix a, DimPoint c, DimPoint b)
    {
        if (b.size() != b.size())
        {
